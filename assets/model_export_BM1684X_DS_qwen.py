...
class BmodelConverter:
    ...
    def run_command(self, command, env):
        try:
            ...
            full_command = f"source {os.path.join(self.tpu_mlir_path, 'envsetup.sh')} && cd {current_dir} && {' '.join(command)}"
            # print(f"{GREEN_COLOR}Executing command: \n{' '.join(command)}{RESET_COLOR}")  # Print the command in green
            # subprocess.run(command, check=True, env=env)

            print(f"{GREEN_COLOR}Executing command: \n{full_command}{RESET_COLOR}")
            subprocess.run(['bash', '-c', full_command], check=True, env=env, cwd=self.tpu_mlir_path)  # 指定 cwd
    ...
    def compile_lm_head(self, env, quantize):
        ...
        if os.path.exists(f"{name}.bmodel"):
            ...
            self.run_command([' '.join(transform_args)], env)
            self.run_command([' '.join(deploy_args)], env)
            self.run_command(['rm', '-f', path], env)
            self.run_command(['rm', '-f', f'{name}_top_f32_all_weight.npz'], env)
    ...
    def combine(self, env):
        ...
        self.run_command([' '.join(combine_args)], env)
    ...
    def compile(self):
        ...
        # # serial execution compile blocks and block_caches
        # for i in range(self.num_layers):
        #     self.compile_block(i, self.env, quantize)
        #     self.compile_block_cache(i, self.env, quantize)
